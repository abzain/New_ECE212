 ...
 * Implementation:
 *   SW1 is the reset.
 *   NOTE: SW2 is shared with LED D4
 ...
 */
#define writeLEDs(val) LATA = (PORTA & 0xFFE4) | (((~(val)) & 0xC)<<1) | ((~(val)) & 0x3)
...
#define RBACK    OC1RS
#define RFORWARD OC2RS
#define LBACK    OC3RS
#define LFORWARD OC4RS
...
void ece212_setup(){
    ...
    //PORT A LEDs or PWM (1=input, 0=output))
    //LEDs    outputs on pins 0, 1, 3, 4
    TRISA = 0x0000;
    //SWs RA4, RB3, 4, 7
    TRISB = 0xFFFF; //all setup as input for simplicity     
